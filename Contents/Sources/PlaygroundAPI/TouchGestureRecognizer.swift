...
internal class TouchGestureRecognizer: UIGestureRecognizer, UIGestureRecognizerDelegate {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent) {
        ...
        // NOTE (ken orr): it's important that we capture the current touches BEFORE we notify the handler, since the delegate may access currentTouches.
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent) {
        ...
        // NOTE (ken orr): it's important that we remove the current touches AFTER we notify the handler, since the delegate may access currentTouches.
