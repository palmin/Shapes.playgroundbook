...
/// A class for specifying and modifying colors of objects on your canvas
///
/// You can choose specific colors such as `Color.orange` or you can
/// create your own by specifiying the red, green, blue and alpha values.
///
...
/// - `colorByAdjustingBrightness(percent: Double)`: Specify the brightness by percent
...
public class Color: _ColorLiteralConvertible, Equatable {
    ...
    ///
    /// `percent` specifies a percent value to lighten the color.
    /// Specify a number from `0.0` to `1.0`
    public func lighter(percent: Double = 0.2) -> Color {
        return colorByAdjustingBrightness(percent: 1 + percent)
    }
    
    /// Specify the alpha (transparency) of a color
    ///
    /// `alpha` specifies a percent value to adjust the transparency.
    /// Specify a number from `0.0` to `1.0`
    ///
    /// 0.0 is completely transparent, and hence invisible
    ///
    ...
    /// Make a color darker
    /// `percent` specifies a percent value to darken the color.
    /// Specify a number from `0.0` to `1.0`
    public func darker(percent: Double = 0.2) -> Color {
        return colorByAdjustingBrightness(percent: 1 - percent)
    }
    
    /// Adjust the brightness of a color by specifying a percent value:
    ...
    /// Pick a random color
    ///
    /// An opaque color with random red, green and blue values will be
    /// selected.
